...
def send_welcome_email(email, api_key, business_name):
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#333">
  ...
  <div style="background:#f9f9f9;padding:32px;border-radius:0 0 12px 12px">
    ...
    <p style="margin-top:12px">Je account is geactiveerd. Ga naar je dashboard om reviews toe te voegen en AI-antwoorden te genereren:</p>
    ...
    <p style="color:#888;font-size:13px">Je API key: <code>{api_key}</code></p>
    <p style="color:#888;font-size:13px;margin-top:20px">Vragen? Mail naar spronken1234@gmail.com</p>
  </div>
</div>"""
    msg = MIMEMultipart('alternative')
    msg['Subject'] = 'Je ReviewHost account is klaar!'
    msg['From']    = gmail
    msg['To']      = email
...
def checkout():
    ...
    if not existing:
        ...
        c.execute('INSERT OR IGNORE INTO customers (id,email,api_key,active,business_name,created_at) VALUES (?,?,?,0,?,?)',
                  (cid, email, api_key, business, datetime.now().isoformat()))
        c.commit()
    c.close()
    print(f'[LEAD] {email} | {business}')
